...
function loginToHub(){
	if (!objMyPermanentDeviceKey)
		return undefined; //#console.log("objMyPermanentDeviceKey not set yet, can't log in");
	if (!objMyTempDeviceKey)
		return undefined; //#console.log("objMyTempDeviceKey not set yet, can't log in");
	if (!my_device_hub)
		return undefined; //#console.log("my_device_hub not set yet, can't log in");
...
function rotateTempDeviceKeyIfCouldBeAlreadyUsed(){
	...
	if (actual_interval > TEMP_DEVICE_KEY_ROTATION_PERIOD + 1000)
		return undefined; //#console.log("woke up after sleep or high load, will skip rotation");
	if (objMyTempDeviceKey.use_count === 0) // new key that was never used yet
		return undefined; //#console.log("the current temp key was not used yet, will not rotate");
...
function rotateTempDeviceKey(){
	if (!saveTempKeys)
		return undefined; //#console.log("no saving function");
	...
	network.findOutboundPeerOrConnect(conf.WS_PROTOCOL+my_device_hub, function onLocatedHubForRotation(err, ws){
		if (err)
			return undefined; //#console.log('will not rotate because: '+err);
		if (ws.readyState !== ws.OPEN)
			return undefined; //#console.log('will not rotate because connection is not open');
		if (!ws.bLoggedIn)
			return undefined; //#console.log('will not rotate because not logged in'); // reconnected and not logged in yet
...
function resendStalledMessages(){
	...
	if (!objMyPermanentDeviceKey)
		return undefined; //#console.log("objMyPermanentDeviceKey not set yet, can't resend stalled messages");
